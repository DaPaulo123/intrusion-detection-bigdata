...
def main():
    ...
    # Loai bo cac dac trung gay ro ri du lieu (Data Leakage) cua UNSW-NB15
    leaky_cols = ['sttl', 'dttl', 'ct_state_ttl']
    proc_train = proc_train.drop(*leaky_cols)
    proc_test = proc_test.drop(*leaky_cols)
    ...
    # ---------------------------------------------------------
    # CHIEN THUAT UNDERSAMPLING (Giam so luong cac nhan pho bien)
    # ---------------------------------------------------------
    MAX_SAMPLES = 5000
    attack_counts = attack_train_data.groupBy("attack_cat_index").count().collect()
    fractions = {}
    for row in attack_counts:
        cat_index = row['attack_cat_index']
        count = row['count']
        if count > MAX_SAMPLES:
            fractions[cat_index] = MAX_SAMPLES / float(count)
        else:
            fractions[cat_index] = 1.0
            
    # Thuc hien lay mau ngau nhien cho tung class dua tren ty le
    attack_train_balanced = attack_train_data.sampleBy("attack_cat_index", fractions, seed=42)
    
    # Ap dung Class Weights len tap da Undersampling
    attack_train_weighted = add_class_weights(attack_train_balanced, label_col="attack_cat_index", weight_col="class_weight")
    ...
    model_stage2 = xgb_stage2.fit(attack_train_weighted)
